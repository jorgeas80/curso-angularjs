...
(function() {
    ...
    angular
        ...
        .directive('regexValidate', regexValidateDirective);


    function regexValidateDirective() {
        return {
            // La directiva será de tipo atributo
            restrict: 'A',

            /**
             * El campo ngModel hace falta. Validamos campos del módelo, no 
             * elementos del DOM
             ...
             * ctrl = este objeto nos da funciones para operar con el objeto del 
             * model.
             **/
            link: function(scope, elem, attr, ctrl) {
                 ...
                 * Vamos a añadir una función a este array que verifique si el 
                 * objeto cumple una propiedad (seguir el formato exigido por la 
                 * expresión regular). 
                 *
                 * Resumen: Los $parsers actúan cuando hay un cambio desde la 
                 * vista hacia el modelo (ej: escribimos algo en un input  
                 * asociado con un campo del modelo por ng-model)
                 **/
                ctrl.$parsers.unshift(function(value) {

                    /**
                     * Establecemos la propiedad "regexValidate" a true/false,
                     * lo que sirve para establecer el campo [formName].
                     * [fieldName].$error.regexValidate a true/false. Así es como 
                     * permitimos que esta validación se pueda usar en el form.
                     **/
                    var valid = regex.test(value);
                    ctrl.$setValidity('regexValidate', valid);

                    /**
                     * Devolvemos el objeto del modelo si se ha cumplido la 
                     * expresión. Si no, devolvemos undefined.
                     **/
                ...
                * Estas funciones se ejecutarán cada vez que un cambio en un 
                * campo del modelo requiera que se renderice ese mismo campo en 
                * algún lugar de la vista. De otra forma: Solo se actualizará el 
                * campo del DOM donde aparezca el modelo tras un cambio en éste 
                * después de que todas estas funciones se ejecuten.
                * 
